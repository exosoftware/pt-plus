...
def migrate(env, version):
    ...
    tag_maps = [
        (
            env.ref("ptplus.tax_tag_iva_dp_1_v").id,
            env.ref("ptplus.tax_tag_iva_dp_1_c").id,
        ),
        (
            env.ref("ptplus.tax_tag_iva_dp_3_v").id,
            env.ref("ptplus.tax_tag_iva_dp_3_c").id,
        ),
        (
            env.ref("ptplus.tax_tag_iva_dp_5_v").id,
            env.ref("ptplus.tax_tag_iva_dp_5_c").id,
        ),
    ]
    for company_id in company_ids:
        for src_tag, dest_tag in tag_maps:
            ...
            repartition_line_ids = repartition_line_ids.filtered(
                lambda r: (
                    r.invoice_tax_id and r.invoice_tax_id.type_tax_use == "purchase"
                )
                or r.refund_tax_id
                and r.refund_tax_id.type_tax_use == "purchase"
            )

            taxes = env["account.tax"]
            if repartition_line_ids:
                taxes = repartition_line_ids.mapped(
                    "invoice_tax_id"
