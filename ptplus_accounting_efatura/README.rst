...
- The merge now refuses the cases where it would have to decide on its own what
  to do with accounting already produced: a duplicate that has been paid, one
  that originated an asset, or one that generated deferral entries. Undo those
  first, or keep that bill instead, which the window lets you do with one
  button.
- On a draft vendor bill, the Odoo banner that spots a duplicate now offers
  "Merge E-Fatura" where a merge is possible.
