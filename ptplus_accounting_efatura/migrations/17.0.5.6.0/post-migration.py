...
def _partner_defaults(env):
    ...
    company dependent res_partner properties (ir_property rows on Odoo 17).

    Only the values set on a partner are carried over (res_id is set): the
    company wide property defaults (res_id NULL) merely repeated the company
    fields handled by _company_defaults(). The rows themselves need no cleanup,
    dropping the fields cascades onto ir_property.
    """
    rate_by_field = {
        f"property_l10n_pt_efatura_{suffix}": rate for suffix, rate in OLD_DEFAULTS
    }
    env.cr.execute(
        """
        SELECT ip.company_id, ip.res_id, imf.name, ip.value_reference
        FROM ir_property ip
        JOIN ir_model_fields imf ON imf.id = ip.fields_id
        WHERE imf.model = 'res.partner'
          AND imf.name IN %s
          AND ip.res_id IS NOT NULL
          AND ip.company_id IS NOT NULL
          AND ip.value_reference IS NOT NULL
        """,
        (tuple(PARTNER_COLUMNS),),
    )
    defaults = {}
    for company_id, res_id, field_name, value_reference in env.cr.fetchall():
        partner_id = int(res_id.split(",")[1])
        record_id = int(value_reference.split(",")[1])
        key = (company_id, partner_id)
        taxes, _product = defaults.setdefault(key, ({}, None))
        if field_name == "property_l10n_pt_efatura_product_id":
            defaults[key] = (taxes, record_id)
        else:
            taxes[rate_by_field[field_name]] = record_id
    return defaults
